...
function AchievementToast({ kind, onDone }) {
  ...
  const A = {
    ...
    class_mago:     { title: 'Classe: Mago 🔮',              sub: 'Balanceado · +10% XP em tudo',         icon: '🔮', color: 'var(--tinta)' },
...
const DAILY_PHRASES = [
  ...
  { text: 'Constância vence talento quando o talento não é constante.', author: 'Legend' },
  { text: 'Aprovação não acontece num dia — acontece todos os dias.', author: 'Legend' },
  ...
  { text: 'A toga não veste quem desiste no meio do caminho.', author: 'Legend' },
  { text: 'A persistência realiza o impossível.', author: 'Provérbio chinês' },
  { text: 'Pequenos passos diários superam grandes saltos esporádicos.', author: 'Legend' },
  ...
  { text: 'Aprovar é decidir, todo dia, não desistir.', author: 'Legend' },
  { text: 'Quem planta constância, colhe aprovação.', author: 'Legend' },
  { text: 'Estude como se a vaga já fosse sua — porque ela está.', author: 'Legend' },
  { text: 'Foco não é fazer mil coisas, é dizer não pra novecentas e noventa e nove.', author: 'Steve Jobs' },
  { text: 'Hábitos diários definem destinos finais.', author: 'Legend' },
  ...
  { text: 'A jornada de mil páginas começa com uma única virada.', author: 'Legend' },
];
...
function BackupSection({ shared, objState, discState, onRestore, onReset, onToast }) {
  ...
  const handleReset = () => {
    ...
    if (typed !== 'RESETAR LEGEND') return;
...
function App() {
  ...
  const [classesModalOpen, setClassesModalOpen] = useState(() => !localStorage.getItem('rol_classes_seen'));
  const prevPetStageRef = useRef(window.DA.getPetStage(shared.xp));
  const prevDragonClassRef = useRef(shared.dragonClass || 'mago');

  // Auto-detecta classe do dragão pelos últimos 14 dias e dispara toast quando muda
  useEffect(() => {
    if (!window.ROL_detectClass) return;
    const detected = window.ROL_detectClass(shared);
    if (detected !== (shared.dragonClass || 'mago')) {
      setShared((s) => ({ ...s, dragonClass: detected }));
      if (prevDragonClassRef.current !== detected) {
        pushToast(`class_${detected}`);
        prevDragonClassRef.current = detected;
      }
    }
  }, [shared.dailyLogs]);
  ...
  const handleMaster = () => {
    setShared(s => {
      const gain = window.ROL_applyClassBonus
        ? window.ROL_applyClassBonus(25, { dragonClass: s.dragonClass || 'mago', type: 'master', isEssential: true })
        : 25;
      return { ...s, xp: s.xp + gain };
    });
  ...
  return (
    <div style={{ position: 'relative', zIndex: 1 }}>
      ...
      <GlobalHeader shared={shared} mode={mode} setMode={setMode} totalPct={totalStats.percentage} onOpenClasses={() => setClassesModalOpen(true)} />

      {/* Sidebar */}
      <nav className="nav-sidebar">
        <div className="nav-sidebar-brand">
          <span style={{ fontSize: 20 }}>🐉</span> Legend
        </div>
        {TABS.map(tab => (
      ...
      <main className="toga-main" style={{ maxWidth: 1200, margin: '0 auto', padding: '22px 24px 100px', position: 'relative' }}>
          ...
          <>
            ...
            <div className="greeting-row" style={{ display: 'grid', gap: 16, gridTemplateColumns: 'minmax(0,1.2fr) minmax(0,1fr)', marginBottom: 16 }}>
              <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
                ...
                <DailyPhrase />
                <PetCompanion xp={shared.xp} sick={isSick} dailyLogs={shared.dailyLogs} dragonClass={shared.dragonClass || 'mago'} />
              </div>
        ...
        {/* ── ABA: DESEMPENHO ── */}
        {activeTab === 'desempenho' && window.DesempenhoTab && (
          <DesempenhoTab shared={shared} setShared={setShared} onToast={pushToast} />
        )}
            ...
            <section style={{ marginBottom: 14 }}>
              <div className="glass" style={{ padding: '18px 20px' }}>
                ...
                <div style={{ fontSize: 11, color: 'var(--text-dim)', marginTop: 10, fontFamily: 'JetBrains Mono, monospace' }}>
                  Rise of a Legend v1.0 · Todos os dados ficam no seu dispositivo
                </div>
              </div>
            </section>
          </>
      ...
      {window.ClassesIntroModal && <ClassesIntroModal open={classesModalOpen} currentClass={shared.dragonClass || 'mago'} onClose={() => { setClassesModalOpen(false); try { localStorage.setItem('rol_classes_seen', '1'); } catch {} }} />}
      {toasts.map(t => <AchievementToast key={t.id} kind={t.kind} onDone={() => setToasts(ts => ts.filter(x => x.id !== t.id))} />)}

      <TweaksPanel title="Tweaks · Rise of a Legend">
        ...
        <TweakSection label="Backup">
          <TweakButton label="💾 Baixar backup" onClick={() => {
            ...
            const blob = new Blob([JSON.stringify(backup, null, 2)], { type: 'application/json' });
            const url = URL.createObjectURL(blob);
            const a = document.createElement('a'); a.href = url; a.download = `rise-of-legend-backup-${new Date().toISOString().slice(0,10)}.json`;
